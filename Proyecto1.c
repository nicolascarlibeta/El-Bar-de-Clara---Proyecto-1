...
void ModificarProductos(){
    ...
    printf("\n");
    MostrarProductos();
    printf("Elija el producto a modificar: ");
    scanf("%d",&P);

    //LEEMOS la PRIMERA LINEA
    p=fopen("productos.bin","rb+");
    
    for(int i=1; i<=P && !feof(p); i++){
        fread(PROD,sizeof(struct Producto),1,p);
    }
    //Indica la POSICIÓN ACTUAL del PUNTERO
    pos=ftell(p)-sizeof(struct Producto); 
    ...
    fwrite(PROD,sizeof(struct Producto),1,p);
    fclose(p); //Lo GUARDAMOS ANTES DE MOSTRAR
...
void ProductoMomV(){
    ...
    switch(i){
        ...
        printf("El producto mas vendido es el/la %s\n",l_recuperar(PRODUCTOS,PMV)->valor);
        break;
        case 2: PmV=ProductoMomVSub(i);
        printf("El producto menos vendido es el/la %s\n",l_recuperar(PRODUCTOS,PmV)->valor);
        break;
